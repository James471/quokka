...
template <typename problem_t> void AdvectionSimulation<problem_t>::advanceSingleTimestepAtLevel(int lev, amrex::Real time, amrex::Real dt_lev, int /*ncycle*/)
{
	...
	const int ncomp = state_old_cc_[lev].nComp();

	// create ghosted working copies for cell-centred states
	amrex::MultiFab state_old_tmp(grids[lev], dmap[lev], ncomp, nghost_cc_);
	amrex::MultiFab state_stage_tmp(grids[lev], dmap[lev], ncomp, nghost_cc_);
	amrex::MultiFab::Copy(state_old_tmp, state_old_cc_[lev], 0, 0, ncomp, 0);
	...
	fillBoundaryConditions(state_old_tmp, state_old_tmp, lev, time, quokka::centering::cc, quokka::direction::na,
	...
	if constexpr (integratorOrder_ == 2) {
		// update ghost zones [w/ intermediate stage stored in state_stage_tmp]
		fillBoundaryConditions(state_stage_tmp, state_stage_tmp, lev, (time + dt_lev), quokka::centering::cc, quokka::direction::na,
				       AMRSimulation<problem_t>::InterpHookNone, AMRSimulation<problem_t>::InterpHookNone);

		// advance all grids on local processor (Stage 2 of integrator)
		{
			auto const &stateInOld = state_old_tmp;
			auto const &stateInStar = state_stage_tmp;
			auto &stateOut = state_stage_tmp;
	...
	amrex::MultiFab::Copy(state_new_cc_[lev], state_stage_tmp, 0, 0, ncomp, 0);
